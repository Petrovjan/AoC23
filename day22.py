...
for m in range(len(data)):
    ...
    if data[m][0][0] != data[m][1][0]:
        for c in range(min(data[m][0][0], data[m][1][0]), max(data[m][0][0], data[m][1][0]) + 1):
            cubes[(c, data[m][0][1], data[m][0][2])] = m
            brick.append([c, data[m][0][1], data[m][0][2]])
        bricks[m] = brick
    elif data[m][0][1] != data[m][1][1]:
        for d in range(min(data[m][0][1], data[m][1][1]), max(data[m][0][1], data[m][1][1]) + 1):
            cubes[(data[m][0][0], d, data[m][0][2])] = m
            brick.append([data[m][0][0], d, data[m][0][2]])
        bricks[m] = brick
    elif data[m][0][2] != data[m][1][2]:
        for e in range(min(data[m][0][2], data[m][1][2]), max(data[m][0][2], data[m][1][2]) + 1):
...
for ffkey, ffval in bricks.items(): #add bricks that don't support any other brick
    if ffkey not in loadbearing.keys() and ffkey not in breakable.keys():
        breakable[ffkey] = -1

print("part1: ",len(breakable))

def waterfall(wbricks, wcubes): #copypasted from p1, just changed variable names for easier debugging
    falling = set()
    wchanged = True
    while wchanged:
        wchanged = False
...
for load in loadbearing.keys():
    ...
    newbricks.pop(load)
    wf = waterfall(newbricks, newcubes)
    result += wf
    if wf > max:
        max = wf

print("part2: ", result, max)